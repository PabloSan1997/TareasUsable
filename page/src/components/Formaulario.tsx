...
export function Formulario() {
    ...
    return (
        <form className="m-auto flex my-8 items-center w-[95%]" onSubmit={subir}>
            <input type="text"
                className="outline-none rounded-md py-1 w-[40%] text-white px-2 text-lg bg-gray-800 placeholder:text-gray-400"
                placeholder="Nueva tarea"
                value={textoForm.tarea}
                onChange={escribirTarea} />
            <input
                type="datetime-local"
                className="outline-none rounded-md ml-5 py-1 text-white px-2 text-lg bg-gray-800 placeholder:text-gray-400"
                placeholder="Fecha vencimiento"
                value={textoForm.fechaFinalizar}
                onChange={escribirFecha}
            />
            <button
                className="w-fit bg-black ml-2 px-5 py-1 rounded-lg text-lg hover:bg-gray-800 text-white active:bg-gray-700"
                type="submit"
            >Subir</button>
        </form>
    )
